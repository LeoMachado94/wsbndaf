...
class IdentificacaoType extends AbstractStructBase
{
    ...
    /**
     * Set idOrigem value
     * @uses \EnumType\IdOrigem::valueIsValid()
     * @uses \EnumType\IdOrigem::getValidValues()
    ...
    public function setIdOrigem($idOrigem = null)
    {
        ...
        if (!\EnumType\IdOrigem::valueIsValid($idOrigem)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\IdOrigem', is_array($idOrigem) ? implode(', ', $idOrigem) : var_export($idOrigem, true), implode(', ', \EnumType\IdOrigem::getValidValues())), __LINE__);
        }
