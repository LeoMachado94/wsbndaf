...
class EstabelecimentoNovoType extends AbstractStructBase
{
     ...
     * @uses \EnumType\IdIdentificacao::valueIsValid()
     * @uses \EnumType\IdIdentificacao::getValidValues()
     * @throws \InvalidArgumentException
     * @param string $idIdentificacao
     * @return \StructType\EstabelecimentoNovoType
     */
    public function setIdIdentificacao($idIdentificacao = null)
    {
        // validation for constraint: enumeration
        if (!\EnumType\IdIdentificacao::valueIsValid($idIdentificacao)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\IdIdentificacao', is_array($idIdentificacao) ? implode(', ', $idIdentificacao) : var_export($idIdentificacao, true), implode(', ', \EnumType\IdIdentificacao::getValidValues())), __LINE__);
        }
     ...
     * This property is removable from request (nillable=true+minOccurs=0), therefore
     * if the value assigned to this property is null, it is removed from this object
     * @uses \EnumType\CoTipoEstabelecimento::valueIsValid()
     * @uses \EnumType\CoTipoEstabelecimento::getValidValues()
     * @throws \InvalidArgumentException
     * @param string $coTipoEstabelecimento
     * @return \StructType\EstabelecimentoNovoType
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\CoTipoEstabelecimento::valueIsValid($coTipoEstabelecimento)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\CoTipoEstabelecimento', is_array($coTipoEstabelecimento) ? implode(', ', $coTipoEstabelecimento) : var_export($coTipoEstabelecimento, true), implode(', ', \EnumType\CoTipoEstabelecimento::getValidValues())), __LINE__);
        }
