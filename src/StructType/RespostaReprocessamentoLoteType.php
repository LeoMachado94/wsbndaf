...
class RespostaReprocessamentoLoteType extends AbstractStructBase
{
     ...
     */
    public function __construct(\StructType\IdentificacaoType $identificacao = null, array $protocolo = array())
    {
        $this
     ...
     * @param \StructType\IdentificacaoType $identificacao
     * @return \StructType\RespostaReprocessamentoLoteType
     */
    ...
    public static function validateProtocoloForArrayContraintsFromSetProtocolo(array $values = array())
    {
        ...
        foreach ($values as $respostaReprocessamentoLoteTypeProtocoloItem) {
            ...
            if (!$respostaReprocessamentoLoteTypeProtocoloItem instanceof \StructType\ProtocoloType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The protocolo property can only contain items of \StructType\ProtocoloType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\ProtocoloType $item
     * @return \StructType\RespostaReprocessamentoLoteType
     */
    public function addToProtocolo(\StructType\ProtocoloType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ProtocoloType) {
            throw new \InvalidArgumentException(sprintf('The protocolo property can only contain items of \StructType\ProtocoloType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->protocolo[] = $item;
        return $this;
    }
