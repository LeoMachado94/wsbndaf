...
class Avaliacoes extends AbstractStructBase
{
    ...
    public static function validateAvaliacaoForArrayContraintsFromSetAvaliacao(array $values = array())
    {
        ...
        foreach ($values as $avaliacoesAvaliacaoItem) {
            ...
            if (!$avaliacoesAvaliacaoItem instanceof \StructType\AvaliacaoType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The avaliacao property can only contain items of \StructType\AvaliacaoType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\AvaliacaoType $item
     * @return \StructType\Avaliacoes
     */
    public function addToAvaliacao(\StructType\AvaliacaoType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\AvaliacaoType) {
            throw new \InvalidArgumentException(sprintf('The avaliacao property can only contain items of \StructType\AvaliacaoType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->avaliacao[] = $item;
        return $this;
    }
