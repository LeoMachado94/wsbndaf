...
class AvaliacaoType extends AbstractStructBase
{
     ...
     * @uses \EnumType\SimNaoEnumType::valueIsValid()
     * @uses \EnumType\SimNaoEnumType::getValidValues()
    ...
    public function setAvAdequacao($avAdequacao = null)
    {
        ...
        if (!\EnumType\SimNaoEnumType::valueIsValid($avAdequacao)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\SimNaoEnumType', is_array($avAdequacao) ? implode(', ', $avAdequacao) : var_export($avAdequacao, true), implode(', ', \EnumType\SimNaoEnumType::getValidValues())), __LINE__);
        }
